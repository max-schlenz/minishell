...
static size_t	word_count_double_and(char const *s, char c)
{
	size_t	i;
	size_t	j;
...
static void	write_string_double_and(char **arr, char *buf_str, char c)
{
	size_t	i;
	size_t	j;
	size_t	k;
	...
	j = 0;
	k = 0;
	while (j < word_count_double_and(buf_str, c))
	{
...
char	**split_double_and(char *s, char c)
{
	...
	if (!buf_str)
		return (NULL);
	arr = malloc(sizeof(char *) * (word_count_double_and(s, c) + 1));
	if (arr)
		write_string_double_and(arr, s, c);
	else
		return (0);
	free(buf_str);
	return (arr);
